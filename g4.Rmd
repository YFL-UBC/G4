...
raw[[2]] <- read_csv(here("20220504_DLD1_NuclightRED_WT_cx5461_pds-hcl.csv")) %>%
  pivot_longer(cols = -(1:2), names_to = "well", values_to = "cell_count") %>%
  left_join(metadata[[2]], by="well") %>% 
  mutate(cell_line = order[1])

raw[[3]] <- read_csv(here("20220504_DLD1_NuclightRED_WT_cx5461_pds-hcl.csv")) %>%
  pivot_longer(cols = -(1:2), names_to = "well", values_to = "cell_count") %>%
  left_join(metadata[[1]], by="well") %>% 
  mutate(cell_line = order[2])

raw[[4]] <- read_csv(here("20220504_DLD1_NuclightRED_WT_cx5461_pds-hcl.csv")) %>%
  pivot_longer(cols = -(1:2), names_to = "well", values_to = "cell_count") %>%
  left_join(metadata[[2]], by="well") %>% 
  mutate(cell_line = order[2])
...
```{r}
raw %>% dplyr::group_by(`Date Time`, Elapsed, compound, dose_nM, cmpd_cat, cell_line) %>%
  summarize(cell_count = mean(cell_count)) %>%
ggplot(aes(x = Elapsed, y = cell_count, color = as.factor(dose_nM))) + 
  geom_line() +
