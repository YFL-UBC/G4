...
![](g4_files/figure-markdown_strict/unnamed-chunk-5-1.png)
    ...
    ggplot(data = newdata, aes(x = Elapsed, y = cell_count)) + 
      ...
      geom_line(data = raw_summary, aes(alpha = 0.2)) +
      facet_grid(rows = vars(compound,as.factor(dose_nM)), cols = vars(cell_line))

![](g4_files/figure-markdown_strict/unnamed-chunk-6-1.png)

    #c("cisplatin;10000;WT", "cisplatin;30000;WT", "cisplatin;10000;BRKO", "cisplatin;30000;BRKO", "cx5461;1000;WT", "cx5461;10000;WT", "cx5461;1000;BRKO", "cx5461;10000;BRKO")

facet by genotype and drug x concentration

    ggplot(data = raw_summary, aes(x = Elapsed, y = cell_count)) + 
      geom_line(aes(color = "red")) +
      facet_grid(cols = vars(compound,as.factor(dose_nM)), rows = vars(cell_line)) +
      scale_color_manual(values=gg_pal)

![](g4_files/figure-markdown_strict/unnamed-chunk-7-1.png)
